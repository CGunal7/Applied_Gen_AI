...
Records
WorldCups.csv
Tournament-level data (year, country, winner, attendance)
20 rows
WorldCupMatches.csv
Match-level data (teams, scores, stage, city, stadium)
850 rows (after cleaning)
WorldCupPlayers.csv
Player-level data (name, position, team, shirt number)
37,784 rows
Source: Kaggle — FIFA World Cup Dataset Coverage: FIFA World Cup tournaments from 1930 to 2014 Access: Publicly available, no API key required. Downloaded manually from Kaggle and placed in the data/ folder.
...
Tournament docs
20
One document per World Cup tournament (year, winner, attendance, goals scored)
Match docs
850
One document per match (teams, scores, stage, city, stadium)
Team stat docs
83
Aggregated stats per team (total games, wins, goals scored, goals conceded)
Head-to-head docs
577
Summary of all matchups between each pair of teams
Total
1,530
All documents combined and indexed in FAISS
